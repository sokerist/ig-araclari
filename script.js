...
// --- YENİ: KORSAN TÜNEL (CORS PROXY) İLE ZORUNLU İNDİRME ---
async function forceDownload(url, isVideo) {
    showToast('İndirme hazırlanıyor, lütfen bekleyin...', 'info');
    try {
        // Instagram sunucuları direkt fetch'i engellediği için araya bir CORS vekili (proxy) koyuyoruz.
        ...
        // Dosya çok büyükse ve proxy tıkanırsa (ya da Safari engellerse) B planı:
        window.open(url, '_blank');
        if (isVideo) {
            setTimeout(() => {
                showToast('İndirme engellendi. Açılan videonun sağ alt köşesinden veya "Paylaş" ikonundan kaydedebilirsiniz.', 'info', true);
            }, 1500);
        }
    }
}
