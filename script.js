...
function attachCinematicLoad(element) {
    ...
    if(element.tagName === 'IMG') {
        ...
        else element.onload = () => element.classList.add('loaded');
    } else if(element.tagName === 'VIDEO') {
        if(element.readyState >= 3) element.classList.add('loaded');
        else element.onloadeddata = () => element.classList.add('loaded');
    }
}
...
function showContextMenu(e, url) {
    ...
    let menuWidth = elements.contextMenu.offsetWidth; let menuHeight = elements.contextMenu.offsetHeight;
...
elements.modalCopyBtn.onclick = () => {
    ...
    let icon = elements.modalCopyBtn.querySelector('i');
    icon.className = 'fa-solid fa-check'; icon.style.color = '#10b981'; showToast('toastCopied', 'success', false);
...
function loadHistory() {
    ...
    let history = JSON.parse(localStorage.getItem('ig_history_v2') || '[]');
    if(history.length === 0) { elements.searchHistory.style.display = 'none'; return; }
    ...
    history.forEach(item => {
        ...
        tag.onclick = () => { elements.mainInput.value = item.user; elements.searchBtn.click(); };
        elements.searchHistory.appendChild(tag);
    });
}
function saveHistory(username, picUrl = null) {
    if(currentMode === 'video' || !username) return;
    let history = JSON.parse(localStorage.getItem('ig_history_v2') || '[]');
    history = history.filter(item => item.user !== username); history.unshift({ user: username, pic: picUrl }); 
...
function updateUI() {
    ...
    document.title = t.title; document.getElementById('mainHeading').textContent = t.heading; document.getElementById('btnText').textContent = t.btnGet;
    ...
    document.getElementById('faq2Q').textContent = t.faq2Q; document.getElementById('faq2A').textContent = t.faq2A;
    document.getElementById('faq3Q').textContent = t.faq3Q; document.getElementById('faq3A').textContent = t.faq3A;
    document.getElementById('footerText').textContent = t.footerText;
    document.getElementById('menuTextOpen').textContent = t.menuOpen; document.getElementById('menuTextCopy').textContent = t.menuCopy;
    document.getElementById('menuTextDl').textContent = t.menuDl; elements.modalDlText.textContent = t.btnModalDl;
    updateModeText();
}
function updateModeText() {
    const t = translations[currentLang];
    const modeMap = { 'profile': { desc: t.descProfile, place: t.placeholderUser }, 'posts': { desc: t.descPosts, place: t.placeholderUser }, 'video': { desc: t.descVideo, place: t.placeholderUrl }, 'story': { desc: t.descStory, place: t.placeholderUser }, 'highlight': { desc: t.descHighlight, place: t.placeholderUser } };
...
elements.searchBtn.addEventListener('click', async function() {
    ...
    const inputValue = cleanInput(rawInputValue); elements.mainInput.value = inputValue; 
    ...
    try {
        ...
        if (currentMode === 'profile') {
            let hdImageUrl = null;
            if (data && data.result && data.result.length > 0 && data.result[0].user) { const userObj = data.result[0].user; hdImageUrl = userObj.hd_profile_pic_url_info?.url || userObj.profile_pic_url_hd || userObj.profile_pic_url; }
            if (hdImageUrl) {
                const proxyUrl = `https://wsrv.nl/?url=${encodeURIComponent(hdImageUrl)}`;
        ...
        let items = [];
        if (currentMode === 'posts') { if (data && data.result && data.result.edges) { items = data.result.edges.map(edge => edge.node); } else if (data && data.edges) { items = data.edges.map(edge => edge.node); } } 
        else { items = Array.isArray(data) ? data : (data.result ? data.result : [data]); }
        ...
        let extractedPic = null; if (items && items.length > 0 && items[0].user && items[0].user.profile_pic_url) { extractedPic = `https://wsrv.nl/?url=${encodeURIComponent(items[0].user.profile_pic_url)}`; }
        saveHistory(inputValue, extractedPic);

        if (currentMode === 'highlight') {
            items.forEach((album, index) => {
                let albumId = album.id || album.highlightId; let title = album.title || "Albüm";
                let rawCoverUrl = album.cover_media?.image_versions2?.candidates?.[0]?.url || album.cover_media?.cropped_image_version?.url || album.cover;
                if(!albumId) return;
        ...
        if (currentMode === 'posts') {
            items.forEach((item, index) => {
                let mediaArray = [];
                if (item.carousel_media && item.carousel_media.length > 0) { mediaArray = item.carousel_media; } else if (item.edge_sidecar_to_children && item.edge_sidecar_to_children.edges) { mediaArray = item.edge_sidecar_to_children.edges.map(e => e.node); } else { mediaArray = [item]; }
                if(mediaArray.length === 0) return;

                let cleanMediaList = mediaArray.map(m => getMediaUrl(m)).filter(m => m.url);
                if(cleanMediaList.length === 0) cleanMediaList = [getMediaUrl(item)]; let coverMedia = cleanMediaList[0];
                
                let captionText = item.caption && item.caption.text ? item.caption.text.substring(0, 80) + "..." : "";
                if (!captionText && item.edge_media_to_caption && item.edge_media_to_caption.edges.length > 0) { captionText = item.edge_media_to_caption.edges[0].node.text.substring(0, 80) + "..."; }
                let likes = item.like_count ? `<i class="fa-solid fa-heart" style="color:#ef4444;"></i> ${item.like_count}` : "";
                if(!likes && item.edge_media_preview_like) likes = `<i class="fa-solid fa-heart" style="color:#ef4444;"></i> ${item.edge_media_preview_like.count}`;
                let comments = item.comment_count ? `<i class="fa-solid fa-comment" style="color:#007acc;"></i> ${item.comment_count}` : "";
                if(!comments && item.edge_media_to_comment) comments = `<i class="fa-solid fa-comment" style="color:#007acc;"></i> ${item.edge_media_to_comment.count}`;
...
async function loadHighlightStories(highlightId) {
    ...
    try {
        const response = await fetch('https://instagram120.p.rapidapi.com/api/instagram/highlightStories', options);
        clearTimeout(timeoutId); hideSkeleton(); let data = await response.json(); 
        let items = Array.isArray(data) ? data : (data.result ? data.result : [data]);
        if (!items || items.length === 0 || !items[0]) { showToast("errHighlight"); return; }
...
document.querySelectorAll('.accordion-header').forEach(button => {
    button.addEventListener('click', () => { const content = button.nextElementSibling; button.classList.toggle('active'); if (button.classList.contains('active')) { content.style.maxHeight = content.scrollHeight + 'px'; } else { content.style.maxHeight = 0; } });
});
...
function updateModalContent() {
    ...
    elements.modalImage.style.display = 'none'; elements.modalVideo.style.display = 'none'; elements.modalVideo.pause(); elements.modalVideo.src = "";
    ...
    } else { elements.modalPrev.style.display = 'none'; elements.modalNext.style.display = 'none'; elements.modalCounter.style.display = 'none'; }
}
...
document.addEventListener('keydown', e => { 
    if (elements.mediaModal.style.display === 'block') {
        if (e.key === 'Escape') closeModal(); if (e.key === 'ArrowRight') nextMedia(); if (e.key === 'ArrowLeft') prevMedia();
    }
});
